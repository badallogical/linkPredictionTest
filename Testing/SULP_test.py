...
fb_df = pd.read_csv('./Testing/DD-Miner_miner-disease-disease.tsv')

G = nx.from_pandas_edgelist(fb_df, 'DOID1', 'DOID2',None,create_using= nx.Graph())
...
test_graph = nx.from_pandas_edgelist(edge_test, 'DOID1', 'DOID2',None, nx.Graph())
...
for index,row in edge_test.iterrows():
    G.remove_edge(row['DOID1'], row['DOID2'])
    removed_edge.append([row['DOID1'], row['DOID2']])
...
non_edges_df = pd.DataFrame([ i for i in nx.non_edges(G) ], columns=["DOID1", "DOID2"]) 

# spliting train , test data fro non edges 
non_edge_train, non_edge_test = train_test_split( non_edges_df, test_size=0.1, random_state=32)
